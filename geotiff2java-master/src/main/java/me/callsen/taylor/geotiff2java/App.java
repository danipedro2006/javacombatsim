...
public class App extends JPanel {
   ...
   // Add overlays in constructor
    ...
    public App() throws Exception {
            ...
            map = ImageIO.read(
                getClass().getResource("/me/callsen/taylor/geotiff2java/Map-army-export.bmp")
            );

            dem = new ElevationModel(
                getClass().getResource("/me/callsen/taylor/geotiff2java/map.tif")
            );
        ...
        // ---- Calibration ----
        MapUtils.initCalibration(
            203, 139, 46.32, 21.78,
            1528, 779, 46.22, 22.08
        );
        ...
        losOverlay = new LOSOverlay(dem, wgsToUtm, map.getWidth(), map.getHeight());

        overlays.add(losOverlay);
        //overlays.add(new LOSOverlay(losLines));
        //overlays.add(new ObstacleOverlay(obstacles));
        ...
        if (mouseX >= 0 && mouseY >= 0) {
            try {
                DirectPosition2D utm =
                    MapUtils.pixelToUTM(mouseX, mouseY, wgsToUtm);
                double z =
                    MapUtils.getElevationAtPixel(dem, mouseX, mouseY);
